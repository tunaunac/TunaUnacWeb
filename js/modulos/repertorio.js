...
window.abrirOverlayInstrumentos = function(canId) {
    ...
    contenedor.style.display = 'block';   // Forzar visibilidad antes de añadir contenido
                    ...
                    <div class="tarjeta-instrumento" onclick="window.abrirPanelRecursos('${cancion.id}', '${inst}', 'repertorio')">
                        ...
                        <div class="tarjeta-instrumento-info">
                            ...
                            <div class="selector-estado-progreso">
                                ...
                                <button class="btn-estado en-progreso ${estado === 'en_progreso' ? 'activo' : ''}" onclick="event.stopPropagation(); window.cambiarProgreso('${cancion.id}', '${inst}', 'sin_iniciar'); window.refrescarOverlayInstrumentos()">En progreso</button>
                                <button class="btn-estado completado ${estado === 'completado' ? 'activo' : ''}" onclick="event.stopPropagation(); window.cambiarProgreso('${cancion.id}', '${inst}', 'sin_iniciar'); window.refrescarOverlayInstrumentos()">Completado</button>
                            </div>
                        </div>
                    </div>`;
            }).join('')}
...
window.abrirPanelRecursos = function(cancionId, instrumento, origen) {
    ...
    // Limpiar y asegurar visibilidad
    contenedor.innerHTML = '';
    ...
    overlay.innerHTML = `
        <button class="btn-volver-overlay" onclick="this.parentElement.remove(); document.getElementById('${origen === 'repertorio' ? 'repertorio-overlays' : 'tareas-overlays'}').style.display='none'; if('${origen}' === 'tareas') window.cancionActual=null;">
...
window.guardarEdicionCancion = async function(canId) {
    ...
    // Forzar cierre del modo edición (por si el snapshot no se dispara)
...
window.refrescarOverlayInstrumentos = function() {
    ...
    // Actualizar tarjetas de instrumento en el overlay de repertorio
    ...
    // Actualizar botones de progreso dentro del overlay de recursos (si está abierto)
