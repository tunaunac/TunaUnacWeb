...
const avatarDiv = document.getElementById('avatar-usuario');
if (avatarDiv) {
    if (window.datosUsuario.foto) {
        avatarDiv.innerHTML = `<img src="${window.datosUsuario.foto}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
    } else {
...
// Actualizar línea de rango y apodo (ej: "Aspirante "Gran Khali"")
const elRol = document.getElementById('rol-usuario');
if (elRol) {
    const rango = window.datosUsuario.rango || '';
    const apodo = window.datosUsuario.apodo ? ` "${window.datosUsuario.apodo}"` : '';
    let textoRol = rango + apodo;
    if (!textoRol.trim()) textoRol = 'Miembro';
    elRol.textContent = textoRol;
}
